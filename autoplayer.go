			...
			case whiteWin:
				...
				case whiteTurn:
					for _, hstpsn := range gm.hst {
						index = ap.index(hstpsn.brd, hstpsn.st)
						for _, appo := range ap[index].pos {
							//
						}
					}
				case blackTurn:
...
// Len returns the length of the auto player.
func (ap autoPlayer) Len() int {
	return len(ap)
}

// Swap two pawn options in an auto player.
